...
def local_training(user_id, save_dir, batch_size, learning_rate, local_epochs, model_dir=None):
    if model_dir == None:
        sp.run("python FL_local_training.py --arch=" + args.arch + " --epochs=" + str(local_epochs) + " --lr=" + str(learning_rate) + " --id=" + str(user_id) + ' --save-dir=' + save_dir + ' --b=' + str(batch_size) + ' --data_path=' + args.data_path, shell=True, stdout=sp.PIPE)
    else:
        sp.run("python FL_local_training.py --arch=" + args.arch + " --epochs=" + str(local_epochs) + " --lr=" + str(learning_rate) + " --id=" + str(user_id) + ' --resume=' + model_dir + ' --save-dir=' + save_dir + ' --b=' + str(batch_size) + ' --data_path=' + args.data_path, shell=True, stdout=sp.PIPE)
...
def trainProcess_selected(user_list, save_dir, model_name, user_selected_num, epoch, local_epochs, learning_rate):
    ...
    if args.start_epoch != 0:
        ...
        Process_pool = mp.Pool(args.process_limit)

        for user_id in user_list:
            Process_pool.apply_async(local_training, args=(user_id, save_dir, args.batch_size, learning_rate, local_epochs, args.save_dir + '/checkpoint_avg_'+ str(epoch - 1) + '.tar',))

        Process_pool.close()
        Process_pool.join()

        
        filename=os.path.join(args.save_dir,'checkpoint_avg_{}.tar'.format(epoch - 1))
        checkpoint = torch.load(filename)
        global_model.load_state_dict(checkpoint['state_dict'])
        global_model.eval()
        total_sample_num = checkpoint['sample_number']
        
    
        for user_id in user_list:
            filename=os.path.join(args.save_dir,'checkpoint_{}.tar'.format(user_id))
            checkpoint = torch.load(filename)
            local_model.load_state_dict(checkpoint['state_dict'])
            local_model.eval()
            sample_number = checkpoint['sample_number']
            
            for key in global_model.state_dict().keys():
                # model.state_dict()[key] =  copy.deepcopy(model.state_dict()[key] + (model_temp.state_dict()[key] - model.state_dict()[key]) * float(sample_number /total_sample_num))
                new_param = global_model.state_dict()[key] + (local_model.state_dict()[key] - global_model.state_dict()[key]) * float(sample_number /total_sample_num) * args.update_lr
                updated_global_model.state_dict()[key].copy_(new_param)
        # index = 0
        # for user_id in user_list:
        #     filename=os.path.join(args.save_dir,'checkpoint_{}.tar'.format(user_id))
        #     checkpoint = torch.load(filename)
        #     sample_number = checkpoint['sample_number']
        #     model_temp_list[index].load_state_dict(checkpoint['state_dict'])
        #     model_temp_list[index].eval()
        #     index +=1

        # for key in model.state_dict().keys():
        #     # model.state_dict()[key] =  copy.deepcopy(model.state_dict()[key] + (model_temp.state_dict()[key] - model.state_dict()[key]) * float(sample_number /total_sample_num))
        #     update_param = sum(model_temp_list[:]) / user_num
        #     new_param = model.state_dict()[key] + (model_temp.state_dict()[key] - update_param) * float(sample_number /total_sample_num)
        #     model.state_dict()[key].copy_(new_param)


        save_checkpoint({
                'epoch': epoch + 1,
                'sample_number': total_sample_num,
                'state_dict': updated_global_model.state_dict(),
            }, filename=os.path.join(args.save_dir,'checkpoint_avg_{}.tar'.format(epoch)))
        ...
        if epoch == 0:
            ...
            Process_pool = mp.Pool(args.process_limit)
            for user_id in user_list:
                Process_pool.apply_async(local_training, args=(user_id, save_dir, args.batch_size, learning_rate, local_epochs,))
            ...
            global_model.eval()
            for user_id in user_list:
                filename=os.path.join(save_dir,'checkpoint_{}.tar'.format(user_id))
                checkpoint_temp = torch.load(filename)
                sample_number = checkpoint_temp['sample_number']
                total_sample_num +=sample_number
            index = 0
            for user_id in user_list:
                filename=os.path.join(save_dir,'checkpoint_{}.tar'.format(user_id))
                checkpoint_temp = torch.load(filename)
                local_model.load_state_dict(checkpoint_temp['state_dict'])
                local_model.eval()
                sample_number = checkpoint_temp['sample_number']
                if index == 0:
                    for key in global_model.state_dict().keys():
                        updated_global_model.state_dict()[key].copy_(local_model.state_dict()[key])
                    index +=1
                else:
                    for key in global_model.state_dict().keys():
                        # model.state_dict()[key] =  copy.deepcopy(model.state_dict()[key] + (model_temp.state_dict()[key] - model.state_dict()[key]) * float(sample_number /total_sample_num))
                        new_param = global_model.state_dict()[key] + (local_model.state_dict()[key] - global_model.state_dict()[key]) * float(sample_number /total_sample_num)
                        updated_global_model.state_dict()[key].copy_(new_param)            
            ...
            save_checkpoint({
                ...
                'sample_number': total_sample_num,
                'state_dict': updated_global_model.state_dict(),
            }, filename=os.path.join(save_dir,'checkpoint_avg_{}.tar'.format(epoch)))
            ...
            for user_id in user_list:
                Process_pool.apply_async(local_training, args=(user_id, save_dir, args.batch_size, learning_rate, local_epochs, save_dir + '/checkpoint_avg_'+ str(epoch - 1) + '.tar',))
            
            Process_pool.close()
            Process_pool.join()

            
            filename=os.path.join(save_dir,'checkpoint_avg_{}.tar'.format(epoch - 1))
            checkpoint = torch.load(filename)
            global_model.load_state_dict(checkpoint['state_dict'])
            global_model.eval()
            total_sample_num = checkpoint['sample_number']
            ...
            #     for key in model.state_dict().keys():
            #         new_param = model.state_dict()[key] + (model_temp.state_dict()[key] - model.state_dict()[key]) * float(sample_number /total_sample_num) *args.update_lr
            #         model.state_dict()[key].copy_(new_param)
...
def main():
    ...
    group_size = selected_user_num
    group_num = int(user_num/selected_user_num)
    ...
    for i in range(args.epochs):
        ...
        data_path = ''
        if user_num == 100:
            if (i%group_num) > (class_num - 1):
                start_index = (i%class_num)*class_num + (int((i%group_num)/class_num)) *group_size
            else:
                start_index = (i%class_num)*class_num
        else:
            start_index = (i%class_num)
        ...
        if args.group == 1:
            ...
            # time.sleep(2)
            if args.iid == 1:
                trainProcess_selected(user_list, save_dir=args.save_dir, model_name=model_name,user_selected_num=selected_user_num,epoch=i, local_epochs=local_epochs, learning_rate=learning_rate)
            else:
                trainProcess_selected(selected_user_list, save_dir=args.save_dir, model_name=model_name,user_selected_num=selected_user_num,epoch=i, local_epochs=local_epochs, learning_rate=learning_rate)
...
if __name__ == '__main__':
    main()
    # total_user_list = np.arange(user_num)
    # user_selection(total_user_list, './data_split_user/train_100_iid_ns', class_num, 10, 0)
    # test()
